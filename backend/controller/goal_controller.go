...
func AddGoal(c *gin.Context) {
	...
	if !purposeOk || !smartSpecificOk || !smartMeasurableOk || !smartAchievableOk || !smartRelevantOk || !smartTimeBoundOk || !userIDOk {
	...
	if err != nil {
		sql := `INSERT INTO goals(id, smartSpecific, smartMeasurable, smartAchievable, smartRelevant, smartTimeBound, purpose, user_id, image_url) VALUES(?, ?, ?, ?, ?, ?, ?, NULL)`
		// sql := `INSERT INTO goals(id, title, text, user_id, image_url) VALUES(?, ?, ?, ?, ?, ?, ?, NULL)`
		_, execErr := db.DB.Exec(sql, req.ID, smartSpecific[0], smartMeasurable[0], smartAchievable[0], smartRelevant[0], smartTimeBound[0], purpose[0], userID[0])
		...
		_, err = db.DB.Exec(sql, req.ID, smartSpecific[0], smartMeasurable[0], smartAchievable[0], smartRelevant[0], smartTimeBound[0], purpose[0], userID[0], imageUrl)
